...
export interface ComboboxProps {
  ...
  /** Options list — can be strings or { value, label } objects */
  options: (string | ComboboxOption)[];
  /** Allow free-text input (not just from options) */
  freeform?: boolean;
...
export function Combobox({
  ...
  // Display value: show search text when typing, otherwise show selected label
  const displayValue = isTyping
    ? search
    : options.find((o) => o.value === value)?.label ?? value;

  // Filter: show all when just opened, filter only when typing
  ...
  // Reset active descendant only when it's out of bounds
  useEffect(() => {
  ...
  useEffect(() => {
    if (activeDescendant < 0 || !listboxRef.current || !filtered[activeDescendant]) return;
    ...
    <div ref={containerRef} className={cn("relative", className)}>
      ...
      >
        ...
        >
          <span
            className={cn(
              "material-symbols-rounded !text-base transition-transform inline-block",
              open && "rotate-180",
            )}
          >
            expand_more
        ...
        {filtered.map((opt, index) => {
          ...
          return (
            ...
            >
              <span>{opt.label}</span>
              {opt.value !== opt.label && (
                <span className="ml-2 text-xs text-on-surface-variant">{opt.value}</span>
              )}
            </li>
          );
        })}
      ...
      {helperText && (
        <p className={cn("text-xs mt-1 px-4", error ? "text-error" : "text-on-surface-variant")}>
          {helperText}
        </p>
      )}
    </div>
