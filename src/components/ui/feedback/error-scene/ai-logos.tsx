...
/**
 * Brand marks for the default FireScene roster (Claude, Gemini, GPT, Grok).
 * Internal to the error scenes — not part of the public API. Each mark fills
 * its container (`h-full w-full`); the scene sizes the wrapper.
 */

export function ClaudeMark() {
  return (
    <svg className="h-full w-full" viewBox="0 0 24 24" fill="#ffffff" xmlns="http://www.w3.org/2000/svg">
...
/** One of Gemini's soft blur filters — they only differ in region and radius. */
function GeminiBlur({
  id,
  x,
  y,
...
export function GeminiMark() {
  // Unique, selector-safe prefix so mask/filter ids survive multiple instances.
  const uid = useId().replace(/:/g, "");
  const id = (key: string) => `gemini-${key}-${uid}`;
  const url = (key: string) => `url(#${id(key)})`;

  return (
    <svg className="h-full w-full" viewBox="0 0 296 298" fill="none" xmlns="http://www.w3.org/2000/svg">
      <mask id={id("star")} x="0" y="0" width="296" height="298" maskUnits="userSpaceOnUse" style={{ maskType: "alpha" }}>
        <path
          fill="#3186FF"
      ...
      <g mask={url("star")}>
        ...
        <g filter={url("green-1")}>
          <path fill="#14BB69" d="M194.5 279.5C172.5 207.5 66 155.667 23 155l121.5 201 50-76.5Z" />
        </g>
        <g filter={url("green-2")}>
          <path fill="#14BB69" d="M196.5 320.5C174.5 248.5 68 196.667 25 196l121.5 201 50-76.5Z" />
        </g>
      </g>
      <defs>
        <GeminiBlur id={id("blue")} x={-69} y={-46} width={464} height={390} deviation={18} />
        <GeminiBlur id={id("gold-1")} x={-99} y={6} width={265} height={273} deviation={32} />
        <GeminiBlur id={id("gold-2")} x={-113} y={12} width={265} height={273} deviation={32} />
        <GeminiBlur id={id("red-1")} x={-41.5} y={-130} width={299.5} height={329} deviation={32} />
        <GeminiBlur id={id("red-2")} x={-45} y={-153} width={299.5} height={329} deviation={32} />
        <GeminiBlur id={id("green-1")} x={-41} y={91} width={299.5} height={329} deviation={32} />
        <GeminiBlur id={id("green-2")} x={-39} y={132} width={299.5} height={329} deviation={32} />
      </defs>
    </svg>
  );
}
...
export function OpenAIMark() {
  return (
    <svg className="h-full w-full" viewBox="0 0 256 260" fill="#ffffff" xmlns="http://www.w3.org/2000/svg">
