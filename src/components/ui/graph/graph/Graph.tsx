...
export const meta: ComponentMeta = {
  name: "Graph",
  description:
    "Force-directed network graph canvas with auto-sizing, drag-to-pin, scroll-to-zoom, fixed nodes, and an imperative replay/fit handle. Pair with GraphAction and GraphSide for a full UI.",
};
...
export const Graph = forwardRef<GraphHandle, GraphProps>(function Graph(
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      step(nodesRef.current, byIdRef.current, edges, s.w, s.h);
      setFrame((f) => f + 1);
  ...
  useEffect(() => {
    ...
    nodesRef.current = seed(nodes, sizeRef.current.w, sizeRef.current.h);
    const map = new Map<string, Sim>();
    for (const n of nodesRef.current) map.set(n.id, n);
    for (const e of edges) {
      const a = map.get(e.source);
  ...
  const handlePointerUp = (e: React.PointerEvent) => {
    if (draggingId) {
      ...
      if (node) {
        ...
        node.pinned = Boolean(src?.fixed || src?.pin);
      }
