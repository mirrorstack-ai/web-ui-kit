...
export function FloatingLabelInput(props: FloatingLabelInputProps) {
  ...
  const type = !multiline ? ((props as InputModeProps).type ?? "text") : undefined;
  ...
  // Show user placeholder only when focused (label has floated up); otherwise use " " for CSS :placeholder-shown trick
  const hasValue = typeof value === "string" ? value.length > 0 : !!value;
  const visiblePlaceholder = focused && !hasValue && userPlaceholder ? userPlaceholder : " ";
  ...
  const sharedClassName = cn(
    ...
    multiline ? (showCounter ? "pt-3 pb-6 resize-none" : "pt-3 pb-2 resize-none") : "py-4",
      ...
      >
        ...
        {showCounter && (
          <span
            ...
            aria-live="polite"
            aria-atomic="true"
            className={cn("absolute bottom-2 right-3 text-xs pointer-events-none", maxLength && charCount > maxLength * 0.875 ? "text-error" : "text-on-surface-variant")}
          >
            {charCount}/{maxLength}
          </span>
        )}
